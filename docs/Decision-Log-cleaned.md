...
Established QuantMind architectural and product decisions must not be silently overridden during implementation.

If implementation requirements conflict with `brain.md`, `PRD.md`, `MVP-Scope.md`, or an accepted decision in this log, the conflict must be explicitly identified and resolved through documentation before implementation proceeds.

## Consequences

AI coding agents and developers must:

- Flag architectural conflicts rather than silently choosing a different approach.
- Update the relevant source-of-truth document when a decision changes.
- Record material changes as a new or superseding decision.
- Avoid introducing competing architecture without an explicit documented decision.
...
This decision reinforces the project's documentation-first and source-of-truth rules.
